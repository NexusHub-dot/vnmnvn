...
int main() {
    Tape onlineA;onlineA.levelID=123;onlineA.fingerprint=111;
    Tape onlineB=onlineA;onlineB.fingerprint=222;
    check(stableMacroFilename(onlineA,3)==stableMacroFilename(onlineB,3),"Online level macro path must not split when m_levelString hash changes");
    check(compatibleLevelIdentity(onlineA,onlineB),"Stable online ID accepts transient data hash changes");
    Tape localA;localA.levelID=0;localA.fingerprint=111;Tape localB=localA;localB.fingerprint=222;
    check(stableMacroFilename(localA,3)!=stableMacroFilename(localB,3),"Local levels keep content-separated macro paths");
    check(!compatibleLevelIdentity(localA,localB),"Local level playback still requires exact content identity");
    Tape localNeg;localNeg.levelID=-1;localNeg.fingerprint=111;Tape localNeg2=localNeg;localNeg2.fingerprint=222;
    check(stableMacroFilename(localNeg,3)!=stableMacroFilename(localNeg2,3),"Non-positive local placeholder IDs must not merge unrelated level data");
    try {
        Recorder recorder;
