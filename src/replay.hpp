...
inline std::string stableLevelToken(Tape const& tape) {
    // Online/official level IDs are the stable identity. m_levelString can be
    // transiently empty or differ between load variants (for example LDM),
    // which previously split one level into multiple macro libraries. Local
    // editor levels often have ID 0, so keep a content hash for those.
    if (tape.levelID > 0) return "id-" + std::to_string(tape.levelID);
    return "local-" + [] (std::uint64_t value) {
        std::ostringstream out; out << std::hex << std::setw(16) << std::setfill('0') << value; return out.str();
    }(tape.fingerprint);
}
inline std::string stableMacroFilename(Tape const& tape, int slot) {
    if (slot < 1 || slot > 20) throw std::runtime_error("Invalid macro slot");
    return stableLevelToken(tape) + "-slot" + std::to_string(slot) + ".pulse";
}
inline std::string legacyMacroFilename(Tape const& tape, int slot) {
    if (slot < 1 || slot > 20) throw std::runtime_error("Invalid macro slot");
    std::ostringstream out;
    out << tape.levelID << '-' << std::hex << std::setw(16) << std::setfill('0') << tape.fingerprint
        << "-slot" << std::dec << slot << ".pulse";
    return out.str();
}
inline bool compatibleLevelIdentity(Tape const& saved, Tape const& current) {
    if (saved.levelID != current.levelID) return false;
    // For stable online/official IDs, allow an in-memory level-string hash to
    // differ. Playback still validates the recorded trajectory every step and
    // will stop immediately on a real data/version mismatch. Local levels have
    // no stable ID and therefore still require an exact content fingerprint.
    return current.levelID > 0 || saved.fingerprint == current.fingerprint;
}
