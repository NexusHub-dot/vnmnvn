...
std::filesystem::path macroPath(pulse::Tape const& tape, int slot) {
    return macroDir() / pulse::stableMacroFilename(tape,slot);
}
std::filesystem::path legacyMacroPath(pulse::Tape const& tape,int slot) {
    return macroDir() / pulse::legacyMacroFilename(tape,slot);
}
std::vector<std::filesystem::path> legacyMacroCandidates(pulse::Tape const& tape,int slot) {
    std::vector<std::filesystem::path> found;
    auto exact=legacyMacroPath(tape,slot);
    if(std::filesystem::is_regular_file(exact))found.push_back(exact);
    if(tape.levelID<=0 || !std::filesystem::exists(macroDir()))return found;
    auto prefix=std::to_string(tape.levelID)+"-";
    auto suffix=fmt::format("-slot{}.pulse",slot);
    for(auto const& entry:std::filesystem::directory_iterator(macroDir())) {
        if(!entry.is_regular_file())continue;
        auto name=entry.path().filename().string();
        if(!name.starts_with(prefix) || !name.ends_with(suffix) || entry.path()==exact)continue;
        try {
            auto candidate=pulse::load(entry.path());
            if(candidate.levelID==tape.levelID)found.push_back(entry.path());
        } catch(...) {}
    }
...
void retireLegacyMacro(std::filesystem::path const& path) {
    if(!std::filesystem::exists(path))return;
    auto retired=path;retired += ".migrated";
    if(std::filesystem::exists(retired))retired += "-"+std::to_string(std::chrono::steady_clock::now().time_since_epoch().count());
    std::filesystem::rename(path,retired);
}
std::filesystem::path resolveMacroPath(pulse::Tape const& tape,int slot,bool migrate=true) {
    auto stable=macroPath(tape,slot);
    if(std::filesystem::is_regular_file(stable))return stable;
    auto candidates=legacyMacroCandidates(tape,slot);
    if(candidates.empty())return stable;
    if(!migrate)return candidates.front();
    // v1.4.7 migration: one online level now has one slot library even if GD
    // supplied different transient m_levelString variants on separate entries.
    // Validate first, then rename the chosen legacy file instead of rewriting
    // it. Rename preserves file size + mtime, so the linked Frame Windows
    // macroRevision remains valid across this storage-key migration.
    (void)pulse::load(candidates.front());
    std::filesystem::create_directories(stable.parent_path());
    auto selectedPath=candidates.front();
    std::filesystem::rename(selectedPath,stable);
    for(auto const& old:candidates) {
        if(old==selectedPath)continue;
        try{retireLegacyMacro(old);}catch(std::exception const& e){log::warn("Could not retire legacy macro {}: {}",old.string(),e.what());}
    }
    log::info("Migrated Pulse slot {} to stable level key {}",slot,pulse::stableLevelToken(tape));
    return stable;
}
std::string nameKey(pulse::Tape const& tape,int slot) {
    return fmt::format("name-{}-{}",pulse::stableLevelToken(tape),slot);
}
std::string legacyNameKey(pulse::Tape const& tape,int slot) {
    return fmt::format("name-{}-{:016x}-{}",tape.levelID,tape.fingerprint,slot);
}
std::string slotName(pulse::Tape const& tape,int slot) {
    constexpr char missing[]="__pulse_missing_name__";
    auto current=Mod::get()->getSavedValue<std::string>(nameKey(tape,slot),missing);
    if(current!=missing)return current;
    auto legacy=Mod::get()->getSavedValue<std::string>(legacyNameKey(tape,slot),missing);
    if(legacy!=missing)return legacy;
    try {
        auto path=resolveMacroPath(tape,slot);
        if(std::filesystem::exists(path)) {
            auto saved=pulse::load(path);
            auto older=Mod::get()->getSavedValue<std::string>(legacyNameKey(saved,slot),missing);
            if(older!=missing)return older;
        }
    } catch(...) {}
...
void saveRecording() {
    ...
    pulse::saveRecordingFile(resolveMacroPath(session.recorder.tape,session.slot),session.recorder.tape);
    session.dirty=false;
...
void startPlay() {
    try {
        ...
        auto path = resolveMacroPath(current, slot);
        ...
        if (!pulse::compatibleLevelIdentity(loaded,current))
            throw std::runtime_error("This recording belongs to different level data");
        if(loaded.levelID!=0 && loaded.fingerprint!=current.fingerprint)
            log::info("Pulse reused stable level ID {} across a transient level-data hash change; trajectory validation remains active",loaded.levelID);
        session.mode = Mode::Off;
        endFrameLink(false, false, true);
        releaseInputs();
        restoreInputSettings();
...
class PulsePopup : public Popup {
    ...
    void updateSlot() {
        ...
        try {
            auto path=resolveMacroPath(m_identity,slot);
            std::string detail;
            if(std::filesystem::exists(path)) {
            ...
            } else detail="Empty slot - Record / Replace starts a take here";
            if(session.mode==Mode::Record)
    ...
    bool setup() {
        if(!attachActiveLevel() || !Popup::init(390.f,315.f))return false;
        m_identity=identity(session.layer);setTitle("Pulse Recordings");
        m_slotLabel=CCLabelBMFont::create("","bigFont.fnt");m_slotLabel->setPosition({195,266});m_slotLabel->setScale(.43f);m_mainLayer->addChild(m_slotLabel);
        button("<",{43,266},menu_selector(PulsePopup::previousSlot));button(">",{347,266},menu_selector(PulsePopup::nextSlot));
        m_name=TextInput::create(260,"Macro name");m_name->setCommonFilter(CommonFilter::Name);m_name->setMaxCharCount(48);m_name->setScale(.8f);m_name->setPosition({159,230});m_mainLayer->addChild(m_name);
        button("Name",{321,230},menu_selector(PulsePopup::rename));
        m_details=CCLabelBMFont::create("","chatFont.fnt");m_details->setPosition({195,184});m_details->setScale(.4f);m_mainLayer->addChild(m_details);
        button("Record / Replace",{79,132},menu_selector(PulsePopup::record),.40f);
        button("Play",{195,132},menu_selector(PulsePopup::play));
        button("Save Now",{311,132},menu_selector(PulsePopup::save),.48f);
        button("Stop",{79,94},menu_selector(PulsePopup::stopNow));
        button("Delete",{195,94},menu_selector(PulsePopup::remove));
        button("Undo Delete",{311,94},menu_selector(PulsePopup::restore),.43f);
        button("Files",{65,55},menu_selector(PulsePopup::files),.48f);
        button("Frame Windows",{195,55},menu_selector(PulsePopup::frames),.40f);
        button("Help",{325,55},menu_selector(PulsePopup::help),.48f);
        auto hint=CCLabelBMFont::create("Quick start: choose a slot > Record / Replace > play > F8 saves. F7 replays. Frame Windows can auto-analyze.","chatFont.fnt");
        hint->setPosition({195,20});hint->limitLabelWidth(360,.33f,.20f);m_mainLayer->addChild(hint);
    ...
    void save(CCObject*) {
        ...
        notify(std::filesystem::exists(resolveMacroPath(m_identity,selected()))?"This recording is already saved":"No take to save. Choose Record / Replace first.");updateSlot();
    }
    void stopNow(CCObject*) {if(attachActiveLevel())stop(true,"Stopped - take saved if present");onClose(nullptr);}
    void remove(CCObject*) {
        try {
            auto path=resolveMacroPath(m_identity,selected());
    ...
    void help(CCObject*) {
        FLAlertLayer::create("Pulse quick guide",
            "<cy>1.</c> Pick a slot. <cy>2.</c> Record / Replace (or F6), then play from the level start. "
            "<cy>3.</c> F8 saves. <cy>4.</c> Play (or F7) replays the selected slot.\n\n"
            "Each online level now uses one stable macro library even if GD loads a different internal level-string variant. "
            "Frame Windows records alongside Pulse and can analyze automatically after a save.","OK")->show();
    }
public:
...
class $modify(PulsePauseLayer, PauseLayer) {
    void customSetup() {
        ...
        auto sprite = ButtonSprite::create("Pulse");
        sprite->setScale(.55f);
...
bool pulse_frame_link::toPulse(pulse_frame_link::Message* msg) {
        if(msg->command==pulse_frame_link::Command::SavedSelection) {
            ...
            try{msg->macroRevision=pulse::revision(resolveMacroPath(identity(msg->layer),msg->slot));}
