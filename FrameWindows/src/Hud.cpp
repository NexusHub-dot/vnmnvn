...
bool Hud::initWithLayer(PlayLayer* pl) {
    ...
    for(auto& row:rows) row=label(this,0.86f);
...
void Hud::render() {
    ...
    bool minimal=mod->getSettingValue<bool>("fwl-minimal-counter") && s.mode!=Mode::Analyzing;
    bool showCounter=mod->getSettingValue<bool>("fwl-show-hud") && (!minimal || run);
    setVisible(enabled && (showCounter || s.mode==Mode::Analyzing));
    ...
    float width=minimal?142.f:164.f;
    float header=minimal?0.f:16.f;
    float footerH=minimal?0.f:16.f;
    float rowStep=13.f;
    float height=header + footerH + (minimal?5.f:8.f) + static_cast<float>(buckets.size())*rowStep;
    ...
    if(opacity>0.f) {
        panel->drawPolygon(rect.data(),4,{0.03f,0.02f,0.08f,opacity*0.65f},0.f,{0,0,0,0});
        if(!minimal) {
            line(panel,{0,height-2.f},{width,height-2.f},{1.f,1.f,1.f,opacity*0.8f},0.75f);
            line(panel,{0,2.f},{width,2.f},{1.f,1.f,1.f,opacity*0.65f},0.75f);
        }
    }

    title->setVisible(!minimal);
    status->setVisible(!minimal);
    totals->setVisible(!minimal);
    footer->setVisible(!minimal);
    ...
    float yCursor = minimal ? height-7.f : height-33.f;
    ...
    for(auto it=start;it!=run->inputs.end() && it->tick<=tick && drawn<100;++it) {
        ...
        if(w.ignored || !shouldMeasure(*it,releases))continue;
