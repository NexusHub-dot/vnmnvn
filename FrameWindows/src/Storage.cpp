...
std::optional<Run> load(std::string const& fingerprint,std::string& error) {
    try {
        ...
        bool currentAnalysis=j["analysisVersion"].asInt().unwrapOr(0)==5;
        ...
        for(auto const& v:inputs.unwrap()) {
            ...
            a.mode=inputModeFromName(modeName);
...
bool exportCSV(Run const& r,std::string& error) {
    try {
        ...
        out << "input,tick,seconds,player,action,early_ticks,late_ticks,window_ticks,censored_above_20,measured,tested_earliest,tested_latest,nominal_ms,width_is_lower_bound,scope,endpoint_tick,early_failure_tick,late_failure_tick,player_mode,ignored,unstable,checkpoint_fallback\n";
        for(size_t i=0;i<r.inputs.size();++i) {
            ...
            out<<','<<(r.nextClick?"next-timing":"full-macro")<<','<<trialEndpoint(r,i,r.nextClick)<<',';
            if(w.measured && w.earlyFailureTick>=0)out<<w.earlyFailureTick;
            out<<',';if(w.measured && w.lateFailureTick>=0)out<<w.lateFailureTick;
            out<<','<<inputModeName(a.mode)<<','<<w.ignored<<','<<w.unstable<<','<<w.checkpointFallback<<'\n';
        }
