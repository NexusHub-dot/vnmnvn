...
int main() {
    ...
    next.inputs={{80,0,true,0,0,InputMode::Cube},{115,0,false,0,0,InputMode::Cube},{160,1,true,0,0,InputMode::Cube},
        {268,0,true,0,0,InputMode::Cube},{400,0,false,0,0,InputMode::Cube},{436,0,true,0,0,InputMode::Cube}};
    require(trialEndpoint(next,0,true)==268,"Cube press endpoint skips replay-only cube release and other player");
    require(trialEndpoint(next,1,true)==268,"Replay-only cube release would point to the next meaningful press");
    ...
    next.inputs[3].mode=InputMode::Wave;next.inputs[4].mode=InputMode::Wave;
    ...
    require(releaseHasIndependentTiming(InputMode::Ship) && releaseHasIndependentTiming(InputMode::Wave) && releaseHasIndependentTiming(InputMode::Robot),
        "ship/wave/robot releases are timing controls");
    require(!releaseHasIndependentTiming(InputMode::Cube) && !releaseHasIndependentTiming(InputMode::Ball) &&
        !releaseHasIndependentTiming(InputMode::Ufo) && !releaseHasIndependentTiming(InputMode::Spider) && !releaseHasIndependentTiming(InputMode::Swing),
        "discrete-mode releases are not separate timings");
    Run filtered;filtered.endTick=900;
    filtered.inputs={{100,0,true,0,0,InputMode::Cube},{130,0,false,0,0,InputMode::Cube},
        {200,0,true,0,0,InputMode::Ship},{230,0,false,0,0,InputMode::Ship},
        {300,0,true,0,0,InputMode::Ball},{330,0,false,0,0,InputMode::Ball},
        {400,0,true,0,0,InputMode::Robot},{430,0,false,0,0,InputMode::Robot}};
    Search filteredSearch(filtered.inputs);filteredSearch.accept(true);
    require(filteredSearch.index==0,"first press remains an analysis target");
    // Finish first press; cube release at index 1 must be skipped automatically.
    filteredSearch.accept(false);filteredSearch.accept(false);
    require(filteredSearch.index==2,"cube release is replay-only and skipped by search");
    filteredSearch.results[0].measured=true;filteredSearch.results[2].measured=true;filteredSearch.results[3].measured=true;
    filteredSearch.results[4].measured=true;filteredSearch.results[6].measured=true;filteredSearch.results[7].measured=true;
    require(filteredSearch.results[1].ignored && filteredSearch.results[5].ignored,"cube/ball releases are explicitly ignored");
    require(!filteredSearch.results[3].ignored && !filteredSearch.results[7].ignored,"ship/robot releases remain measurable");
    filtered.windows=filteredSearch.results;
    auto filteredCounts=count(filtered,filtered.endTick,true);
    require(filteredCounts.releases==2,"histogram counts only relevant ship/robot releases");
    require(trialEndpoint(filtered,2,true)==230,"ship press endpoint is its next control transition");
    require(trialEndpoint(filtered,3,true)==300,"ship release endpoint is the next control transition");
